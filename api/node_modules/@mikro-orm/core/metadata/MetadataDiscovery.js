...
class MetadataDiscovery {
    ...
    findReferencingProperties(meta, metadata) {
        for (const meta2 of metadata) {
            const prop2 = meta2.relations.find(prop2 => {
                return prop2.reference !== enums_1.ReferenceType.SCALAR && prop2.type === meta.className;
            });
            if (prop2) {
    ...
    initCustomType(meta, prop) {
        ...
        if (!prop.customType && [enums_1.ReferenceType.ONE_TO_ONE, enums_1.ReferenceType.MANY_TO_ONE].includes(prop.reference) && this.metadata.get(prop.type).compositePK) {
            prop.customTypes = [];
            for (const pk of this.metadata.get(prop.type).getPrimaryProps()) {
                if (pk.customType) {
                    prop.customTypes.push(pk.customType);
                    prop.hasConvertToJSValueSQL || (prop.hasConvertToJSValueSQL = !!pk.customType.convertToJSValueSQL && pk.customType.convertToJSValueSQL('', this.platform) !== '');
                    prop.hasConvertToDatabaseValueSQL || (prop.hasConvertToDatabaseValueSQL = !!pk.customType.convertToDatabaseValueSQL && pk.customType.convertToDatabaseValueSQL('', this.platform) !== '');
                }
            }
        }
    ...
    async initColumnType(meta, prop, path) {
        ...
        if (prop.reference === enums_1.ReferenceType.SCALAR && prop.type == null && prop.columnTypes) {
            const mappedType = this.getMappedType(prop);
