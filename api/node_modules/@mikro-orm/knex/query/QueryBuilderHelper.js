...
class QueryBuilderHelper {
    ...
    mapper(field, type = enums_1.QueryType.SELECT, value, alias) {
        ...
        if (customExpression) {
            const bindings = core_1.Utils.asArray(value).slice(0, ret.match(/\?/g)?.length ?? 0);
    ...
    processJoinClause(key, value, params, operator = '$eq') {
        ...
        if (core_1.Utils.isOperator(key, false) && core_1.Utils.isPlainObject(value)) {
            const parts = Object.keys(value).map(k => this.processJoinClause(k, value[k], params, key));
        ...
        if (core_1.Utils.isPlainObject(value) && Object.keys(value).every(k => core_1.Utils.isOperator(k, false))) {
            const parts = Object.keys(value).map(op => this.processJoinClause(key, value[op], params, op));
    ...
    getQueryOrderFromObject(type, orderBy, populate) {
        ...
        Object.keys(orderBy).forEach(k => {
            ...
            core_1.Utils.splitPrimaryKeys(k).forEach(f => {
    ...
    mapData(data, properties, convertCustomTypes) {
        if (!properties) {
            return data;
        }
